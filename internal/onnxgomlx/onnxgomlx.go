...
// When enabled, Concat ops that grow the sequence dimension of KV caches are replaced
// with DynamicUpdateSlice, placing the new entry at the given position instead of appending.
// Call with nil to disable padded KV cache mode.
func (m *Model) WithPaddedKVCache(positionIDs *Node) onnx.Model {
